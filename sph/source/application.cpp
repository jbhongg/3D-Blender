...
namespace sph
{
	...
	void application::initialize_opengl()
	{
		...
		compute_shader_handle = compile_shader("../shader/intergrate1.comp.spv", GL_COMPUTE_SHADER);
		...
		if (scene_id == 0)
		{
			for (auto i = 0, x = 1, y = 1, z= 1; i < SPH_NUM_PARTICLES; i++)
			{
				initial_position[i].x = -0.625 + SPH_PARTICLE_RADIUS * 2 * x;
				initial_position[i].y = 1 - SPH_PARTICLE_RADIUS * 2 * y;
				initial_position[i].z = 1 - SPH_PARTICLE_RADIUS * 2 * z;
				z++;
				if (z >= 30)
				{
					z = 1;
	...
	/*	glm::mat4 view;
		...
		glDrawArrays(0, 0, SPH_NUM_PARTICLES);

		//float sphereRadius = powf((3 * MASS) / (4 * M_PI * REST_DENSITY), 1.0f / 3.0f);
		//for (int i = 0; i < SPH_NUM_PARTICLES; i++) {
		//	glPushMatrix();
		//	glTranslatef(initial_position[i].x, mParticles[i].mPosition.y, mParticles[i].mPosition.z);
		//	glutSolidSphere(SPH_PARTICLE_RADIUS, 30, 30);
		//	glPopMatrix();
		//}
